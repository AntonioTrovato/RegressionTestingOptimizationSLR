...
# --- Taxonomy substrings for SELECTION (all lowercase) ---
TAXONOMY_KEYS = [
    "integer", "data-", "symbolic", "dynamic", "graph",
    "textual", "sdg", "path", "modification", "firewall",
    "cluster", "design"
]

# Algorithm family substrings (same rules as before)
...
def filter_ps_selection(df):
    """Filter only Y == 'PS' and Z contains 'selection' (case-insensitive)."""
    result = []
...
def main():
    ap = argparse.ArgumentParser(description="List (taxonomy, algorithm) -> paper_i (with [multi-obj]) for SELECTION papers.")
    ...
    df = load_df(in_path)
    filtered = filter_ps_selection(df)
    ...
    # Print as numbered list in the required format
